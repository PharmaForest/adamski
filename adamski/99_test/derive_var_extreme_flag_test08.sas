...
/*expected output*/
data advs_exp;
  length studyid $5 usubjid $4 paramcd $6 avisit $10 ablfl $1;
  informat adt yymmdd10.;
  format adt yymmdd10.;
...
/*output by the macro*/	
data advs_temp advs_weight advs_others;
    set advs;
...
/* set the two outputs together */
data advs_out;
	set advs_out1 advs_out2 advs_others;
run;
...
%mp_assertdataset(
  ...
  desc		= (%nrstr(%derive_var_extreme_flag))[test08] Compare expected and test results, 	/* description */
